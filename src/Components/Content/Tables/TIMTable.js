...
class TIMTable extends Component {
  ...
  sendDownloadRequest(id) {
    const url = window.base_url + "getTIM";
  ...
  sendTIM = (
    ...
    const url = window.base_url + "/addTIM";
    const formData = new FormData();
    console.log(Allen);
      ...
      this.sendTIM(
        ...
        .then((TIMResponse) => {
          ...
          if (TIMResponse.status < 400) {
            this.getDataOnDataset(sessionStorage.getItem("datasetName")).then(
              (response) => {
                if (response.status < 400) {
  ...
  renderAddRunHeader = () => {
    return (
      <thead>
        <tr>
          <td width={"8%"}>PAA-Bins-Interpolation-Method</td>
          <td width={"5%"}>Min. Vertical Support (%)</td>
          <td width={"5%"}>Max Gap (Integer)</td>
          <td width={"5%"}>No. of Allen Relations</td>
          <td width={"5%"}>Epsilon (Integer)</td>
          <td width={"5%"}>Max TIRP Length (Integer)</td>
          <td width={"5%"}>Index Same</td>
          <td width={"5%"}>Mine</td>
        </tr>
      </thead>
    );
  ...
  renderAddRunData = () => {
    return JSON.parse(sessionStorage.DiscretizationTable).rows.map(
      (iter, index) => {
        let Disc = "Disc" + index;
        ...
        return (
          <tr key={index}>
            ...
            <td>
              <Form.Control id={MinVSInput} type={"text"}></Form.Control>
            </td>
            <td>
              <Form.Control id={MaxGapInput} type={"text"}></Form.Control>
            </td>
            <td>
              <ButtonGroup id={"Allen" + index} toggle={true}>
                <ToggleButton
                  checked={
                    this.state.Allen.has(sIndex)
            ...
            <td>
              <Form.Control id={EpsilonInput} type={"text"}></Form.Control>
            </td>
            <td>
              <Form.Control id={maxTirpLenInput} type={"text"}></Form.Control>
            </td>
            <td>
              <ButtonGroup id={"Class" + index} toggle={true}>
                <ToggleButton
                  checked={
                    this.state.Class.has(sIndex)
  ...
  renderExistingRunsHeader = () => {
    return (
      <thead>
        <tr>
          <td>PAA-Bins-Interpolation-Method</td>
          <td>Min. Vertical Support (%)</td>
          <td>Max Gap (Integer)</td>
          <td>No. of Allen Relations</td>
          <td>Epsilon (Integer)</td>
          <td>Max TIRP Length (Integer)</td>
          <td>Index Same</td>
          <td>Download class0</td>
          <td>Download class1</td>
          <td>Download Both</td>
        </tr>
      </thead>
    );
    ...
    return JSON.parse(sessionStorage.TIMTable).rows.map((iter, index) => {
      return (
        <tr key={index}>
          ...
          <td width={"5%"}>{iter["VerticalSupport"]}</td>
          <td width={"5%"}>{iter["MaxGap"]}</td>
          <td width={"5%"}>{iter["numRelations"]}</td>
          <td width={"5%"}>{iter["epsilon"]}</td>
          <td width={"5%"}>{iter["maxTirpLength"]}</td>
          <td width={"5%"}>{iter["indexSame"]}</td>
          <td width={"12%"}>
            {
              <Button
