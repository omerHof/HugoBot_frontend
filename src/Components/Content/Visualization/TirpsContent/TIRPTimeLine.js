...
class TIRPTimeLine extends Component {
  ...
  drawTimeLine = () => {
    ...
    interval.push(
      this.state.symbols[0] + " - " + service.getDiffBetweenDates(date1, date2)
    );
    interval.push(date1);
    interval.push(date2);
    data.push(interval);
    ...
    for (var i = 1; i < this.state.symbols.length; i++) {
      ...
      interval.push(
        this.state.symbols[i] +
          " - " +
          service.getDiffBetweenDates(date1, date2)
      );
      interval.push(date1);
      interval.push(date2);
      data.push(interval);
  ...
  drawTimeLineForDisc = () => {
    ...
    if (!this.state.currTirp._TIRP__exist_in_class0) {
      ...
      let date2Class1 = service.getDateForSymbol(
        this.state.durationOfFirstIntervalClass1
      );
    ...
    for (var i = 1; i < this.state.symbols.length; i++) {
      ...
      if (!this.state.currTirp._TIRP__exist_in_class0) {
        ...
        let date1Class1 = service.getDateForSymbol(
          offsetClass1 + this.state.timesClass1[j]
        );
        let date2Class1 = service.getDateForSymbol(
          offsetClass1 + this.state.timesClass1[j + 1]
        );
        duration +=
          " / " + service.getDiffBetweenDates(date1Class1, date2Class1);
      } else {
        duration += " / 0 " + window.dataSetInfo.timestamp;
      }
