...
class VMapFile extends Component {
    ...
    if (e.target.value === "") {
      ...
      for (let key of this.state.map.keys()) {
        ...
        if (map.get(key).localeCompare(e.target.value) === 0) {
          ...
          document
            .getElementById(key)
            .setAttribute("bgcolor", this.DUPLICATE_COLOR);
        }
      }
      if (flag) {
  ...
  renderTableRow = (placeholder, idx) => {
    return (
      <tr key={idx.toString()}>
        <td id={"id" + idx}>{placeholder}</td>
        <td bgcolor={this.UNFILLED_COLOR} id={"name" + idx + "_td"}>
          <Form.Control
            bgcolor={this.UNFILLED_COLOR}
            id={"name" + idx}
            onChange={this.recolorCell}
          />
        </td>
        <td>
  ...
  getVariablesForVMapRequest = (dataset_id) => {
    const url = window.base_url + "/getVariableList?dataset_id=" + dataset_id;
    const config = {
      headers: {
  ...
  sendVMapCreate(csv, datasetName) {
    const url = window.base_url + "/steptwocreate";
  ...
  sendVMapUpload(file, datasetName) {
    const url = window.base_url + "/steptwo";
    const formData = new FormData();
    ...
    return (
      <Card>
        <Card.Header className={"bg-hugobot"}>
          <Card.Text className={"text-hugobot"}>VMap File</Card.Text>
        </Card.Header>
        <Card.Body>
          <ButtonGroup>
            <Button
              className="btn-hugobot"
              onClick={this.changeViewCreate}
              type={"button"}
            >
          ...
          <br />
          <br />
          {this.state.onDisplay.localeCompare("Upload") === 0 &&
            this.renderUpload()}
          <Table>
            {this.state.onDisplay.localeCompare("Create") === 0 &&
